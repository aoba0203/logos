...
def get_dataset():
    img_list = []
    for img_file in glob.glob('./symbol/*.jpg'):
        img = np.expand_dims(np.asarray((Image.open(img_file)).resize((128, 128))), axis=0)
        img = (img / 127.5) - 1
        img_list.append(img)
...
class WGANGP():
    ...
    def build_generator(self):
        ...
        model.add(Dense(128 * 16, activation="relu"))
...
#                 axs[i,j].imshow(gen_imgs[cnt, :,:,0], cmap='gray')
        ...
        file_name = 'images/{}_{}.jpg'.format(str_time, epoch)
...
wgan = WGANGP()
wgan.train(epochs=30000000, batch_size=4, sample_interval=1000)
